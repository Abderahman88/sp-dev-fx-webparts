  ...
  public async getEvent(siteUrl: string, listId: string, eventId: number): Promise<IEventData> {
    ...
    try {
      ...
      returnEvent = {
        ...
        EventDate: moment(event.EventDate).subtract((siteTimeZoneHours), 'hours').toDate(),
        EndDate: moment(event.EndDate).subtract((siteTimeZoneHours), 'hours').toDate(),
      ...
      if (results && results.Row.length > 0) {
        ...
        for (event of results.Row) {
          ...
          events.push({
            ...
            EventType: event.EventType,
            title: await this.deCodeHtmlEntities(event.Title),
            EventDate: isAllDayEvent ? new Date(event.EventDate.slice(0, -1)) : moment(event.EventDate).subtract((siteTimeZoneHours), 'hour').toDate(),
            EndDate: isAllDayEvent ? new Date(event.EndDate.slice(0, -1)) : moment(event.EndDate).subtract(siteTimeZoneHours, 'hour').toDate(),
